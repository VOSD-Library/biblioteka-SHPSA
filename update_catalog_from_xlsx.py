...
ОБЛОЖКИ КНИГ:
    ...
    1) Автоматически по инвентарному номеру (самый простой способ):
       ...
       положите туда фото обложки в формате .jpg с именем как инвентарный
       номер книги, например:
           covers/ШПСА 00545.jpg
       Сайт сам найдёт нужный файл — регистр букв (ШПСА / шпса) и
       пробел/подчёркивание (ШПСА 00545 / ШПСА_00545) значения не имеют,
       сайт проверит основные варианты написания. Ничего в xlsx менять
       не нужно.
...
def clean_cover(raw):
    """Возвращает значение столбца 'Обложка', только если оно ПОХОЖЕ на имя
    файла картинки или ссылку. В реальных xlsx-файлах иногда встречаются
    лишние данные в этом столбце (обрывки других столбцов, случайно
    попавшие туда при копировании) — если такое значение принять как есть,
    сайт будет пытаться загрузить несуществующую 'обложку' и ничего не
    покажет, хотя настоящий файл обложки (по инвентарному номеру) есть.
    Поэтому здесь — простая проверка на здравый смысл, а не слепое доверие
    содержимому ячейки."""
    s = clean_str(raw)
    if not s:
        return ""
    ...
    if COVER_EXT_RE.search(low):
        return s
    return ""  # не похоже на имя файла картинки или ссылку — игнорируем
...
def main():
    ...
    if ignored_covers:
        print(f"⚠ В столбце 'Обложка' у {ignored_covers} книг(и) найдено значение, не похожее "
              f"на имя файла картинки или ссылку (например, обрывок текста) — оно проигнорировано. "
              f"Для этих книг обложка подставится автоматически по инвентарному номеру, если "
              f"есть подходящий файл в папке covers/.")
